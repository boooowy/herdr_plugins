...
| `C` | **コメント投稿** — diff 行:インラインコメント / `v` 選択中:**複数行コメント** / スレッド上:返信 / それ以外:PRコメント。投稿先はフッタに「C:返信→著者 L15」等で常に表示。エディタが popup で開き、保存して閉じると投稿（空なら中止）。**画面には自動反映** |
...
- コメントは罫線ではなく背景色でエリアを表現し、返信は ↳ とインデントで入れ子を示します。
  Comments タブのインラインコメントは、**コメントが付いた hunk 全体を diff 形式で表示**し、
  該当行の直下にコメントを埋め込みます（diff から位置が特定できないコメントはファイル末尾に
  L番号ラベル付きで表示）。カーソル下のスレッドは背景色が明るくなり（`focus_bg`）、
  `C` の投稿先はフッタに常に表示されます。
