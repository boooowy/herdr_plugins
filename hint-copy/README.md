...
4つのモードがあり、目的に応じて切り替えます:

| モード | 入り方 | ラベルが付く対象 | 向いている用途 |
| --- | --- | --- | --- |
| **ワード**(既定) | 起動時 / Tab | 画面上の**すべての単語**(英数字+`word_chars` の3文字以上の並び)。ラベルは単語の直前に挿入され文字を隠さない | 画面に見えている任意の単語をサッとコピー |
| **トークン** | Tab | URL・パス・SHA・IP・クォート文字列などの**パターン抽出**。`https://example.com/app` が丸ごと1候補 | URLやパスを**丸ごと**コピー |
| **行** | Space | 各行(左マージンにラベル) | 行単位・複数行のコピー |
| **copy** | `[` / 専用アクション | ラベルなし。vim風カーソル選択 | スクロールバックを遡る自由範囲のコピー |

ワードモードでは `https://example.com/app` は `https` / `example.com` / `app` に分割されます。丸ごと欲しいときだけ Tab でトークンモードへ。
...
ラベルが付かない単語があるのは次の場合です:

- `word_min_len`(既定3)未満の短い単語(`go` や2文字の日本語など)
- ユニークな単語の種類がラベルの上限(`alphabet` 24文字の2文字組合せ = **576**)を超えた分。初出順に割り当てるため、超えた場合は画面下部の単語から欠ける
- ラベル挿入で行が右にずれ、画面右端からあふれた部分
...
max_candidates = 100   # トークンモードのユニーク候補の上限(ワードモードは alphabet² まで拾う)
