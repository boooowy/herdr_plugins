...
候補の範囲は `match_bg` の背景色でハイライトされるので、**どこまでコピーされるかがラベル入力前に見えます**。2文字ラベルの1文字目を打つと、外れた候補のハイライトは消えて残りだけが光り、フッターに残候補数(候補が1つに絞れたときはコピーされる内容のプレビュー)が表示されます。

### ワードモード(単語ラベル)

パターンに当たらない文字列を拾いたいときは Tab でワードモードへ。画面上の単語(英数字と `word_chars` の文字からなる3文字以上の並び。長さは `word_min_len` で変更可)すべてにラベルが付きます。もう一度 Tab でトークンモードに戻ります。
